...
from idc.models import instance_source, Base, Version, Collection, Patient, Study, Series
...
def update_version_hashes(args, sess, parent_class, table, dones):
    ...
    for parent in parents:
        if True:
            ...
            prev_hash = parent_hashes[args.source.value]
            child_hashes = [child.hashes._asdict()[args.source.name] for child in parent.collections if child.hashes._asdict()[args.source.name]]
            parent_hashes[args.source.value] = get_merkle_hash(child_hashes) \
                if child_hashes else ''
            if parent.hashes[args.source.value] != parent_hashes[args.source.value]:
                parent.hashes = parent_hashes
                progresslogger.info(f'{args.id}: {n}of{cnt}: {parent_class} {parent.version}: {prev_hash}-->{parent_hashes[args.source.value]} ')

                if prev_hash:
                    errlogger.error(f'{args.id}: {n}of{cnt}: {parent_class} {parent.version}: {prev_hash}-->{parent_hashes[args.source.value]} ')
                changed += 1
                # if not changed%100 :
                #     # sess.commit()
...
def update_collection_hashes(args, sess, parent_class, table, dones):
    ...
    for parent in parents:
        if True:
            parent_hashes = list(parent.hashes)
            prev_hash = parent_hashes[args.source.value]
            child_hashes = [child.hashes._asdict()[args.source.name] for child in parent.patients if child.hashes._asdict()[args.source.name]]
            parent_hashes[args.source.value] = get_merkle_hash(child_hashes) \
                if child_hashes else ''
            if parent.hashes[args.source.value] != parent_hashes[args.source.value]:
                parent.hashes = parent_hashes
                progresslogger.info(f'{args.id}: {n}of{cnt}: {parent_class} {parent.uuid}: {prev_hash}-->{parent_hashes[args.source.value]} ')

                if prev_hash:
                    errlogger.error(f'{args.id}: {n}of{cnt}: {parent_class} {parent.uuid}: {prev_hash}-->{parent_hashes[args.source.value]} ')
                changed += 1
                # if not changed%100 :
                #     # sess.commit()
                #     progresslogger.info(f'{like}: {args.id}: Changed {changed}, {time.time() - strt}')
...
def update_patient_hashes(args, sess, parent_class, table, like, dones):
    ...
    for parent in parents:
        if parent.uuid not in dones:
            parent_hashes = list(parent.hashes)
            prev_hash = parent_hashes[args.source.value]
            child_hashes = [child.hashes._asdict()[args.source.name] for child in parent.studies if child.hashes._asdict()[args.source.name]]
            parent_hashes[args.source.value] = get_merkle_hash(child_hashes) \
                if child_hashes else ''
            if parent.hashes[args.source.value] != parent_hashes[args.source.value]:
                if prev_hash:
...
def update_study_hashes(args, sess, parent_class, table, like, dones):
    ...
    for parent in parents:
        if parent.uuid not in dones:
            parent_hashes = list(parent.hashes)
            prev_hash = parent_hashes[args.source.value]
            child_hashes = [child.hashes._asdict()[args.source.name] for child in parent.seriess if child.hashes._asdict()[args.source.name]]
            parent_hashes[args.source.value] = get_merkle_hash(child_hashes)  \
                if child_hashes else ''
            if parent.hashes[args.source.value] != parent_hashes[args.source.value]:
                parent.hashes = parent_hashes
    ...
    for parent in parents:
        if parent.uuid not in dones:
            ...
            child_hashes = [instance.hash for instance in parent.instances \
                            if instance.source == args.source]
            parent_hashes[args.source.value] = get_merkle_hash(child_hashes) \
                if child_hashes else ''
            if parent.hashes[args.source.value] != parent_hashes[args.source.value]:
                parent.hashes = parent_hashes
