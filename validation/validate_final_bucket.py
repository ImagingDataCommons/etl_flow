...
def validate_bucket(cur, args):
    ...
    WHERE tcia_api_collection_id = 'ACRIN-FMISO-Brain'
    """)
    while True:
        ...
        for row in rows:
            ...
            blob_name_dash = f"{uuid[0:8]}-{uuid[8:12]}-{uuid[12:16]}-{uuid[16:20]}-{uuid[20:32]}.dcm"
            if storage.Blob(bucket=bucket, name=blob_name_dash).exists(client):
                rootlogger.info('%s: %s renamed blob exists', n, blob_name_dash)
            else:
                errlogger.error('%s: %s renamed blob not found', n, blob_name_dash)
                blob_name = f'{row["gcs_url"].rsplit("/")[-1]}.dcm'
                if storage.Blob(bucket=bucket, name=blob_name).exists(client):
                    rootlogger.info('%s: %s original blob exists', n, blob_name)
...
if __name__ == '__main__':
    ...
    done_fh = logging.FileHandler('{}/logs/validate_final_bucket_done.log'.format(os.environ['PWD']))
