...
# Generate a reindex manifest, changing Phantom FDA blobs to bucket idc-open-data
...
def gen_instance_manifest(args):
    ...
    WITH
      ...
      SELECT
        ...
        CONCAT('gs://idc-open-data/', se_uuid, '/', i_uuid, '.dcm', ', s3://idc-open-data/', se_uuid, '/', i_uuid, '.dcm') new_urls,
        CONCAT('gs://idc-open-cr/', se_uuid, '/', i_uuid, '.dcm', ', s3://idc-open-data-cr/', se_uuid, '/', i_uuid, '.dcm') old_urls
      FROM
      ...
      ON
        1=1
      WHERE idc_version = {args.version} AND collection_id = "Phantom FDA"
    )
    SELECT
      ...
      i_size size,
      '*' acl,
      CONCAT('[', STRING_AGG(new_urls, ', ' ORDER BY SPLIT(new_urls, '/')[offset(3) ]), ']') url,
      ...
      CONCAT('[', STRING_AGG(old_urls, ', ' ORDER BY SPLIT(old_urls, '/')[offset(3) ]), ']') previous_url
    FROM
      mult
    GROUP BY
      sop_instance_uid,
