...
def deprecate_v1_v6(client, idc_version, mitigation_id):
    query = f"""
UPDATE `{settings.DEV_MITIGATION_PROJECT}.idc_v{idc_version}_dev.wsi_metadata` AS i
SET
    redacted = True,
    mitigation = '{mitigation_id}'
FROM `{settings.DEV_MITIGATION_PROJECT}.mitigations.{mitigation_id}` AS d
...
FROM (
    ...
    FROM `{settings.DEV_MITIGATION_PROJECT}.idc_v{idc_version}_dev.{table_prefix}{parent}` {parent_alias}
...
if __name__ == "__main__":
    ...
    for idc_version in range(3,19):
        progresslogger.info(f'Revising version {idc_version}')
        if idc_version <= 6:
            deprecate_v1_v6(client, idc_version, mitigation_id="m1")
            progresslogger.info(f'Revised version {idc_version}')
        else:
            if idc_version <= 12:
                prefix = 'wsi_'
            else:
            ...
            deprecate_level(client, idc_version, prefix, mitigation_id="m1", parent="series", child="instance",
