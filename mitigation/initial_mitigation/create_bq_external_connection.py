...
def create_connection(version):
    ...
    cloud_sql = bigquery_connection_v1.types.CloudSqlProperties(
        ...
        database = f'idc_v{version}',
        type_ = type_,
        credential = credential
    )

    connection = bigquery_connection_v1.types.Connection(
        name = f'projects/{settings.DEV_PROJECT}/locations/{settings.BQ_REGION}/connections/etl_federated_query_idc_v{version}',
        friendly_name = f'etl_federated_query_idc_v{version}',
        description = '',
        cloud_sql = cloud_sql
    )
    ...
    request  = bigquery_connection_v1.CreateConnectionRequest(
        ...
        connection_id = f'etl_federated_query_idc_v{version}',
