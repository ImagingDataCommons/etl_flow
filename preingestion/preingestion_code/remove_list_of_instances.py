...
def remove_instances(client, args, sess, instances):
    query = f"""
DELETE FROM idc_instance 
WHERE sop_instance_uid IN ({','.join(instances)})
    """
    result = sess.execute(query).fetchall()
    return


def remove_series(client, args, sess, instances):
    remove_instances(client, args, sess, instances)
...
def remove_studies(client, args, sess, instances):
    remove_series(client, args, sess, instances)
...
def remove_patients(client, args, sess, instances):
    remove_studies(client, args, sess, instances)
...
def remove_collections(client, args, sess, instances):
    remove_patients(client, args, sess, instances)
