...
class All_mtm(All):
    ...
    def collections(self):
        ...
        # We get the collection_id_map separately because at this point it is not in the V5 DB.
        # In the future it will be in the "previous" DB so can be joined in the above query
        map = {row.collection_id: row.idc_collection_id for row in self.v6_sess.query(Collection_id_map).all()}

        # Now generate entries for new collections..those not in collection_id_map
        # already
        for collection in pre_metadata:
            if not collection in map:
                map[collection] = uuid4()
                row = Collection_id_map(collection_id=collection, idc_collection_id=map[collection])
                self.v6_sess.add(row)
                self.v6_sess.commit()

        # Now we restructure patient_metadata so that it is  indexed by the idc_collection_id
        collection_metadata = {
            map[key]: {
                'collection_id': key,
                'rev_idc_version': pre_metadata[key]['rev_idc_version'],
                'min_timestamp': pre_metadata[key]['min_timestamp'],
                'max_timestamp': pre_metadata[key]['max_timestamp'],
                'sources': pre_metadata[key]['sources'],
                'hashes': pre_metadata[key]['hashes']
            } for key in pre_metadata
        }
    ...
    def studies(self, patient):
        study_metadata = {row.study_instance_uid: {
            ...
            'sources': list(row.sources._asdict().values()),
            'hashes': list(
                dict({key: val if not val is None else "" for key, val in row.hashes._asdict().items()}).values())
        } for row in self.sess.query(Study).where(Study.submitter_case_id==patient.submitter_case_id).all()}
    ...
    def instances(self, series):
        instance_metadata = {row.sop_instance_uid: {
            ...
            'hash': row.hash
        } for row in self.sess.query(Instance).where(Instance.series_instance_uid==series.series_instance_uid).all()}
    ...
    def collection_was_updated(self, collection):
        ...
        for source in self.sources:
            ...
            updated |= collection.hashes[source.value] != self.collection_metadata[collection.idc_collection_id]['hashes'][source.value]
        return updated
    ...
    def patient_was_updated(self, patient):
        updated=False;
        for source in self.sources:
            updated |= patient.hashes[source.value] != self.patient_metadata[patient.submitter_case_id]['hashes'][source.value]
        return updated
    ...
    def study_was_updated(self, study):
        updated=False;
        for source in self.sources:
            updated |= study.hashes[source.value] != self.study_metadata[study.study_instance_uid]['hashes'][source.value]
        return updated
    ...
    def series_was_updated(self, series):
        updated=False;
        for source in self.sources:
            updated |= series.hashes[source.value] != self.series_metadata[series.series_instance_uid]['hashes'][source.value]
        return updated


    def instance_was_updated(self, instance):
        updated = instance.hash != self.instance_metadata[instance.sop_instance_uid]['hash']
        return updated
