...
def wait_done(response, args, sleep_time, verbose=True):
    ...
    while True:
        result = get_dataset_operation(args.gch_project, args.gch_region, args.gch_dataset, operation)
        if verbose:
            # print("{}".format(result))
            try:
...
def import_buckets(args):
    ...
    try:
        dataset = get_dataset(args.gch_project, args.gch_region, args.gch_dataset)
    except HttpError:
        # Dataset doesn't exist. Create it.
        response = create_dataset(args.gch_project, args.gch_region, args.gch_dataset)

    try:
        datastore = get_dicom_store(args.gch_project, args.gch_region, args.gch_dataset, f'{args.gch_dicomstore}')
    except HttpError:
        # Datastore doesn't exist. Create it
        datastore = create_dicom_store(args.gch_project, args.gch_region, args.gch_dataset, f'{args.gch_dicomstore}')
    pass
        ...
        if '*' in bucket:
            ...
            for bucket in buckets:
                if not bucket in dones:
                    try:
                        ...
                        response = import_dicom_instances(args.gch_project, args.gch_region, args.gch_dataset,
                                                          f'{args.gch_dicomstore}', content_uri)
            ...
            if not bucket in dones:
                try:
                    ...
                    content_uri = '{}/*/*'.format(bucket)
                    response = import_dicom_instances(args.gch_project, args.gch_region, args.gch_dataset,
                                    args.gch_dicomstore, content_uri)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--src_buckets', default=['idc-conversion-outputs-rms-test-2'],
            help="List of buckets from which to import.")
    parser.add_argument('--gch_project', default='idc-dev-etl')
    parser.add_argument('--gch_region', default='us-central1')
    parser.add_argument('--gch_dataset', default='idc')
    parser.add_argument('--gch_dicomstore', default='idc-conversion-outputs-rms-test-2')
