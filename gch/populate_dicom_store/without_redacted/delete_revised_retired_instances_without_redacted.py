...
from idc.models import Base, Version, Patient, Study, Series, Instance, Collection, CR_Collections, Defaced_Collections, Excluded_Collections, Open_Collections
...
def delete_instances(args, sess, dicomweb_sess):
    ...
    # open_collections, cr_collections and defaced_collections.
    try:
        uids = json.load(open('logs/revised_uids.txt'))
    except:
        ...
        uids = [{'study_instance_uid':row.study_instance_uid, 'series_instance_uid':row.series_instance_uid,
                 'sop_instance_uid':row.sop_instance_uid } for row in rows if row.collection_id in collections]
        with open('logs/revised_uids.txt','w') as f:
            json.dump(uids,f)
    n=0
    for row in uids:
    ...
    # We delete those now.
    try:
        uids = json.load(open('logs/retired_uids.txt'))
    except:
        rows =  sess.query(Collection.collection_id, Study.study_instance_uid, Series.series_instance_uid, Instance.sop_instance_uid).\
        ...
        uids = [{'study_instance_uid':row.study_instance_uid, 'series_instance_uid':row.series_instance_uid,
                 'sop_instance_uid':row.sop_instance_uid } for row in rows if row.collection_id in collections]
        with open('logs/retired_uids.txt','w') as f:
            json.dump(uids,f)
    n=0
    for row in uids:
        if not row['sop_instance_uid'] in done_instances:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--staging_bucket', default='idc-dev-open')
