    ...
    for ofile in ofiles:
        ...
        if (file_ext == '.json') and not collec.startswith(CURRENT_VERSION):
            ...
            try:
                ...
                errlogger.error(f'Error for {collec}: {exc}')
...
def load_all(project, dataset, version, last_dataset, last_version, srcfiles):
    client = bigquery.Client(project=project)
    dataset_id = project + "." + dataset
    ...
    if srcfiles is None:
        ...
        htan = addTables(project, dataset, version, "HTAN", None, HTAN_TABLES, HTAN_SRCS, "HTAN_Participant_ID", False,
                         last_dataset, last_version)
        cptac = addTables(project, dataset, version, "CPTAC", None, ["clinical"], [CPTAC_SRC], "submitter_id", False,
                          last_dataset, last_version)
        tcga = addTables(project, dataset, version, "TCGA", None, ["clinical"], [TCGA_SRC], "case_barcode", False,
                         last_dataset, last_version, [TCGA_REC_SRC])
...
if __name__ == "__main__":
    srcfiles = None
    # srcfiles = ['cc_radiomics_phantom_3_control_settings.json']
    load_all(DEFAULT_PROJECT, DATASET, CURRENT_VERSION, LAST_DATASET, LAST_VERSION, srcfiles)
# checkData()
