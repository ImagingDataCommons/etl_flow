...
def mergeAcrossBatch(clinJson, coll, ptRowIds, attrSetInd, colsAdded, csrc, offset):
    ...
    if 'archive' in clinJson[coll][csrc][attrSetInd][0]:
        clinJson[coll]['mergeBatch'][attrSetInd + offset]['srcs'][0] = \
        clinJson[coll]['mergeBatch'][attrSetInd + offset]['srcs'][0] + clinJson[coll][csrc][attrSetInd][0]['archive']
    ...
    for batchSetInd in range(1, len(clinJson[coll]['cols'][attrSetInd])):
        ...
        for header in clinJson[coll]['cols'][attrSetInd][batchSetInd]['headers']:
            ...
            if header == cptRow:
                ckey = ptRow
                clinJson[coll]['mergeBatch'][attrSetInd + offset]['headers'][ptRow].append(
                    clinJson[coll]['cols'][attrSetInd][batchSetInd]['headers'][cptRow])
            else:
                clinJson[coll]['mergeBatch'][attrSetInd + offset]['headers'][header].append(
                    clinJson[coll]['cols'][attrSetInd][batchSetInd]['headers'][header])
            curInd = len(clinJson[coll]['mergeBatch'][attrSetInd + offset]['headers'][ckey]) - 1
            clinJson[coll]['mergeBatch'][attrSetInd + offset]['headers'][ckey][curInd]['filenm'] = \
            clinJson[coll]['srcs'][attrSetInd][batchSetInd]['filenm']
        ...
        if 'archive' in clinJson[coll][csrc][attrSetInd][batchSetInd]:
            clinJson[coll]['mergeBatch'][attrSetInd + offset]['srcs'][batchSetInd] = \
            clinJson[coll]['mergeBatch'][attrSetInd + offset]['srcs'][batchSetInd] + \
            clinJson[coll]['srcs'][attrSetInd][batchSetInd]['archive']
        clinJson[coll]['mergeBatch'][attrSetInd + offset][csrc][batchSetInd].append(
            clinJson[coll][csrc][attrSetInd][batchSetInd]['filenm'])
        # join data frames
            ...
            for k in range(len(clinJson[coll]['mergeBatch'])):
                if 'ptId' in clinJson[coll]['mergeBatch'][k]:
                    ...
                    if table_name in hist:
                        ...
                        if (hist_post_process_src_tr != post_process_src_tr) or (
                                post_process_src_current_md5 != hist[table_name]['post_process_src_updated_md5']):
                            sumDic['idc_version_table_prior'] = \
                            hist[table_name]['idc_version_table_updated'].rsplit('/')[0]
                            sumDic['post_process_src_prior_md5'] = \
                            hist[table_name]['post_process_src_updated_md5'].rsplit('/')[0]
            ...
            if not wJson in clinJson[coll]:
                ...
                if 'tcia_api' in clinJson[coll]:
                    if 'case_id' in srcInfo:
                        add_tcia_case_id(clinJson[coll]['mergeBatch'][attrSetInd + offset], clinJson[coll]['tcia_api'],
                                         srcInfo['case_id'])
                    else:
                        add_tcia_case_id(clinJson[coll]['mergeBatch'][attrSetInd + offset], clinJson[coll]['tcia_api'],
                    ...
                    if 'case_id' in srcInfo:
                        ...
                        add_tcia_case_id(clinJson[coll]['mergeBatch'][attrSetInd + offset], None,
                                         clinJson[coll]['case_id'])
...
def parse_dict(fpath, collec, ndic, indx, coll):
    ...
    if (ndic["form"] == "adrenal"):
    ...
    elif (ndic["form"] == "spinal"):
        for index, row in df.iterrows():
            column = row['Column Title ']
            column = formatForBQ([[column]], True)[0]
            if column == 'case_identifier_number':
                column = 'case'
            if column == 'age':
                column = 'age_y'
                ...
                if '=' in optCol:
                    ...
                    if (len(optColA) > 1):
                        ...
                        for k in range(len(optCode)):
                            data_dict[column]['opts'].append(
                                {"option_code": optCode[k], "option_description": optDesc[k]})
        ...
        for index, row in df.iterrows():
            ...
            if len(row['column_counts']) > 0:
                data_dict[column]['opts'] = []
                optJ = json.loads(
                    row['column_counts'].replace('\'', '"').replace('False', '"False"').replace('True', '"True"'))
                for nkey in optJ:
