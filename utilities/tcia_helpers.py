...
def get_access_token(auth_server = NBIA_AUTH_URL):
    ...
    try:
        ...
        logging.error(f"Error getting access token: {exc}")
        logging.error(f"Response content: {result.content}")
        raise RuntimeError(f"Failed to get access token from {auth_server}. Response: {result.content}") from exc
...
def series_drill_down(series_ids, server="" ):
    ...
    data = {"list": [id for id in series_ids]}
    headers = dict(
        Authorization=f'Bearer {access_token}')
    result = requests.post(
        url,
    ...
    # except:
    #     pass
    # old_res = json.loads(result.stdout.decode())
    # return json.loads(result.stdout.decode())

def get_collection_descriptions(collection=None):
    if collection == 'NLST':
        access_token, refresh_token = get_access_token(NLST_AUTH_URL)
        headers = dict(
            Authorization=f'Bearer {access_token}'
    ...
    if not collection:
        ...
        nlst_description = get_collection_descriptions('NLST')
...
def get_collection_license_info():
    table = get_collection_descriptions()
...
def get_all_tcia_metadata(type, query_param='', count=''):
    if query_param:
...
if __name__ == "__main__":
    ...
    sm = get_TCIA_series_metadata('1.3.6.1.4.1.14519.5.2.1.9203.8273.200351476752279282009286093500')
    d = get_all_tcia_metadata(type="download-requirements")

    i = get_collection_license_info()
    l = get_collection_descriptions()
    l = get_collection_descriptions(collection='TCGA-BRCA')
    l = get_collection_descriptions(collection='NLST')
    i = get_TCIA_instance_uids_per_series('Colorectal-Liver-Metastases', '1.3.6.1.4.1.14519.5.2.1.9203.8273.200351476752279282009286093500')



    a = get_access_token(auth_server = NBIA_AUTH_URL)
    d = get_tcia_instance_hash("1.3.6.1.4.1.14519.5.2.1.160539987621162982128991523753352035099", a[0])
