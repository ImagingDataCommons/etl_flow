...
def create_dest_bucket(args):
    ...
    try:
        ...
        print("Error creating bucket {}: {}".format(args.dst_bucket_name, result), flush=True)
        raise
...
def rsync_buckets(args):
    ...
    for bucket in args.src_buckets:
        print("Copying {}".format(bucket), flush=True)
        try:
            result = run(['gsutil', '-u', args.dst_project, '-m', 'rsync', f'gs://{bucket}/*',
                          f'gs://{args.dst_bucket_name}'])
            print("   {} synced, results: {}".format(bucket, result), flush=True)
            if result.returncode:
                print('Copy {} failed: {}'.format(result.stderr), flush=True)
                return {"bucket": bucket, "status": -1}
        except:
