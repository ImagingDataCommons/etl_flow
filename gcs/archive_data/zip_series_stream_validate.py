...
def get_undone_series(args: object) -> object:
    ...
    having src_bucket not in ('idc-dev-excluded', 'idc-dev-redacted')
    ...
    for more_args in iter(zip_queue.get, 'STOP'):
        ...
        try:
            ...
            if validate_zip(args, series, src_bucket, dst_bucket) == 0:
                ...
                rate = round(series.se_size / elapsed_time / 10 ** 6, 1)
                progresslogger.info(
                    f'p{args.pid:03}:    {series_index}:{series.se_uuid}, {round(series.se_size / pow(2, 20), 2)}MB, {rate}MB/s, val:{round(elapsed_time, 2)}s')
                successlogger.info(series.se_uuid)
            else:
                errlogger.error('p{args.pid:03}:    Validation failure: {series_index}:{series.se_uuid}')

        except Exception as exc:
            errlogger.error(f'p{args.pid:03}:    {series.se_uuid}: {exc}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_processes', default=128)
