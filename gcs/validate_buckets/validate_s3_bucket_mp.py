...
def worker(input, args):
    ...
    for series_uuids, n in iter(input.get, 'STOP'):
            for series_uuid in series_uuids:
                try:
                    # The 'Prefix' parameter restricts the results to keys starting with that string
                    for page in paginator.paginate(Bucket=bucket_name, Prefix=series_uuid):
                        if 'Contents' in page:
                            for obj in page['Contents']:
                                successlogger.info(obj['Key'])
                        else:
                            print(f"No files found with prefix: {series_uuid}")
                    progresslogger.info(series_uuid)
                except Exception as exc:
                    errlogger.error(f'p{args.id}: Error {exc}')
        ...
        for page in paginator.paginate(Bucket=bucket_name, Delimiter='/'):
            if 'CommonPrefixes' in page:
                for folder in page['CommonPrefixes']:
                    found_series.append(folder['Prefix'])
        with open(f"{settings.LOG_DIR}/found_series.json", "w") as f:
            json.dump(found_series, f)

    return set([series_uuid.split('/')[0] for series_uuid in found_series[1:]])
...
def check_all_instances_mp(args, max_version=settings.CURRENT_VERSION):
    ...
    found_blobs = set([blob for blob in open(args.found_blobs).read().splitlines() if blob.endswith('.dcm')])
    ...
    if found_blobs == expected_blobs:
        progresslogger.info(f"Bucket {args.bucket} has the correct set of blobs")
    else:
