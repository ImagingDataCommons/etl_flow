...
def get_revised_blobs(dst_dir, dst_bucket, collection_id, url, existing_blob_hashes):
    ...
    #     errlogger.error(f'Aspera download failed')
    #     exit -1
    # files = os.listdir(f'{dst_dir}/{collection_id}')
    # files.sort()
    client = storage.Client()
    ...
    for file in files:
        if file["basename"] not in dones:
            file_path = f'{collection_id}/{file}'
            if file in existing_blob_hashes:
                # If we have this file already, see if it's hash has changed
                with open(f'{dst_dir}/{file_path}', "rb") as f:
                    m = hashlib.md5()
                    while batch := f.read(BATCH_SIZE):
                        m.update(batch)
                    hash = m.hexdigest()
                    if hash == existing_blob_hashes[file]:
                        progresslogger.info(f'{file} unchanged')
                        successlogger.info(file)
                    else:
                        progresslogger.info(f'{file} revised')
                        res = run(['gsutil', '-m', 'cp', f'{dst_dir}/{file_path}', f'gs://{dst_bucket}/{file_path}'])
                        if res.stderr is None:
                            successlogger.info(file)
                        else:
                            errlogger.error(f'Transfer to gcs failed: {file}')

            else:
                progresslogger.info(f'{file} is new')
                 # Copy to GCS
                res = run(['gsutil', '-m', 'cp', f'{dst_dir}/{file_path}',
                           f'gs://{dst_bucket}/{file_path}'])
                if res.stderr is None:
                    successlogger.info(file)
                else:
                    errlogger.error(f'Transfer to gcs failed: {file}')
    ...
    # Delete the file
    run(["rm", '-r', f'{dst_dir}/{collection_id}'])
...
def get_collection(dst_dir, dst_bucket,  collection_id, url):
    existing_blob_hashes = get_existing_blob_hashes(collection_id, 'idc-source-data-cmb')
    get_revised_blobs(dst_dir, dst_bucket, collection_id, url, existing_blob_hashes)
...
if __name__ == '__main__':
    ...
    mount_point = '/mnt/disks/idc-etl-aspera/'
    ...
    for collection_id, url in collections.items():
        ...
        # dest_dir = f'{mount_point}/{collection_id}'
        # pathlib.Path(dest_dir).mkdir(exist_ok=True)
        get_collection(mount_point, dst_bucket, collection_id, url)
