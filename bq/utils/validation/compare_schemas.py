...
def compare_schemas(pdp_table_name, idc_table_name, has_urls):
    ...
    if pdp_table.schema == idc_table.schema:
        ...
        if len(idc_schema) != len(pdp_schema):
            ...
            for key in idc_schema:
                if key in pdp_schema:
                    if idc_schema[key][1].field_type != pdp_schema[key][1].field_type or \
                            idc_schema[key][1].mode != pdp_schema[key][1].mode:
                        are_equal = False
                        break
                else:
                    are_equal = False
                    break
            if are_equal:
                progresslogger.info(f'{pdp_table_name} == {idc_table_name} excepting descriptions')
            return
...
def compare_tables(args, ref_name, table_name, has_urls, min_version, has_view):
    if int(dataset_version) >= min_version:
        ...
        if has_view:
            ...
            if project == 'idc-dev-etl' and int(dataset_version) >= 10:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ref_project', default="idc-dev-etl", help='Project of reference datasets')
    ...
    for dataset_version in [str(i) for i in range(10,14)]:
        ...
        for table_name, has_urls, min_version, has_view in steps:
            ...
            #     continue
            if has_view:
                if args.ref_project == 'idc-dev-etl' and int(dataset_version) >=10:
                    ref_name = f'{args.ref_project}.idc_v{dataset_version}_pub.{table_name}'
                else:
