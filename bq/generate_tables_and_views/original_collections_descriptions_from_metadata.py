...
from original_collections_metadata.gen_original_data_collection_metadata_table import get_all_descriptions


def gen_table():
    bq_client = bigquery.Client(project='idc-dev-etl')

    collections = [[key, value['description']] for  key, value in get_all_descriptions(bq_client, None).items()]
    ...
    for index, row in enumerate(collections):
        ...
        while (anchor_start := description.find('<a', start)) >= 0 :
            ...
            if new_anchor.find('.gov') == -1:
                ...
                new_anchor = new_anchor[0:gt] + ' data-toggle="modal" data-target="#external-web-warning"' + new_anchor[gt:]
    ...
    load_BQ_from_json(bq_client, 'idc-dev-etl', 'idc_v16_dev', 'original_collections_descriptions', json_object,
                      aschema=None, \
