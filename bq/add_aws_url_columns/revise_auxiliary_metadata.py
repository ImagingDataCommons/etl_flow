...
def add_aws_url_column(args):
    client = bigquery.Client()
    # query = f"""
    # ALTER TABLE `{args.project}.{args.dataset}.auxiliary_metadata`
    # ADD COLUMN aws_url VARCHAR
    # """
    table = client.get_table(f'{args.project}.{args.dataset}.auxiliary_metadata')  # Make an API request.

    original_schema = table.schema
    new_schema = original_schema[:]  # Creates a copy of the schema.

    # Add aws_url if we do not have one already
    if next((item for item in original_schema if item.name == 'aws_url'), -1) == -1:
        new_schema.append(bigquery.SchemaField("aws_url", "STRING"))
