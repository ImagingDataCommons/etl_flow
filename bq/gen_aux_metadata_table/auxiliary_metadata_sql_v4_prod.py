...
WITH
  ...
  FROM
    `idc-dev-etl.idc_v4.instance` AS i
  JOIN
  ...
  FROM
    series_hashes AS se
  JOIN
    `idc-dev-etl.idc_v4.study` AS st
  ON
  ...
  FROM
    study_hashes AS st
  JOIN
    `idc-dev-etl.idc_v4.patient` AS p
  ON
  ...
  FROM
    patient_hashes AS p
  JOIN
    `idc-dev-etl.idc_v4.collection` AS c
  ON
  ...
  SELECT
    ...
    vh.hash_all
  FROM
    `idc-dev-etl.idc_v4.version` AS v
  JOIN
  ...
  SELECT
    ...
    license_short_name
  FROM
    `idc-dev-etl.idc_v4.original_collections_metadata`
  UNION ALL
  SELECT
    DOI,
    license_url,
    ...
    license_short_name
  FROM
    `idc-dev-etl.idc_v4.analysis_results_metadata` )
SELECT
  --      3 AS idc_version,
...
--  CONCAT('gs://', IF
--    (c.collection_id='NLST', 'idc-nlst-open', '{gcs_bucket}'), '/', i.uuid, '.dcm') as gcs_url,
...
ON
  se.series_instance_uid = i.series_instance_uid
LEFT JOIN
  `idc-dev-etl.idc_v4.excluded_collections` AS ex
ON
